...
async def manual_fetch(request: Request, db: Session = Depends(get_db)):
    ...
    """
    # Fetch from 5 most recent runs with no item limits to get ALL listings
    inserted, skipped = await fetch_and_store_latest(db, runs_to_scan=5, items_per_run_limit=None)
